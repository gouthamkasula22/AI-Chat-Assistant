...
from utils.logger import get_logger, log_ai_response, log_performance, log_security_event
...
class AdvancedAIService:
    ...
    def __init__(self):
        ...
        self._initialize_learning_service()
    ...
    def _register_models(self):
        ...
        try:
            ...
            logger.error("Failed to register AI models: {e}")

    def _initialize_learning_service(self):
        """Initialize the learning service for intelligent model selection."""
        try:
            self.learning_service = LearningService(feedback_manager=self.feedback_manager)
            logger.info("Learning service initialized successfully")
        except Exception as error:
            logger.error("Failed to initialize learning service: {e}")
    ...
    def generate_response(self, messages: List[Dict[str, str]], session_id: str,
        ...
        # Log the request
        logger.info("AI request started: style={style.value}, preferred_model={preferred_model}, session={session_id[:8]}...")

        try:
            # Validate input
            if not messages or len(messages) == 0:
                log_security_event("empty_message_request", {
            ...
            if input_length > 10000:  # Very long input
                log_security_event("excessive_input_length", {
                    "session_id": session_id,
            ...
            # Use learning service to get optimal model if available
            if self.learning_service and not preferred_model:
                available_model_names = self.get_available_model_names()
                optimal_model = self.learning_service.get_optimal_model(
                    conversation_style=style.value,
                    available_models=available_model_names
                )
                if optimal_model:
                    preferred_model = optimal_model
                    logger.info("Learning service recommended model: {optimal_model} from available: {available_model_names}")
            ...
            if response.success:
                ...
                logger.info("AI response successful: {model_name} -> {output_length} chars in {total_time:.3f}s")

                # Record model performance for learning service
                if self.learning_service:
                    self.learning_service.record_model_performance(
                ...
                # Log failed AI response
                total_time = time.time() - start_time
                log_ai_response(
                    model=preferred_model or "unknown",
                    input_length=input_length,
                    output_length=0,
                    response_time=total_time,
                    session_id=session_id,
                ...
                logger.warning("AI response failed: {error_msg} in {total_time:.3f}s")
            ...
            log_performance(
                ...
                duration=total_time,
                model=response.model_used if response.success else "failed",
                input_length=input_length,
                output_length=len(response.content) if response.success else 0,
                session_id=session_id,
                style=style.value
            )
            ...
            total_time = time.time() - start_time
            logger.error("Error in advanced AI service: {error}")
            self.usage_analytics['failed_requests'] += 1
    ...
    def get_available_model_names(self) -> List[str]:
        """
        Get list of model names that are currently available and working.
    ...
    def set_preferred_model(self, model_name: str) -> bool:
        ...
        if model_name in self.model_manager.models:
            ...
            logger.info("Set preferred model to: {model_name}")
            return True
    ...
    def add_message_feedback(self, message_id: str, feedback_type: str,
                           feedback_value: Any, session_id: str) -> bool:
        """
        Add user feedback for a specific message.

        Args:
            message_id (str): Unique identifier for the message
            feedback_type (str): Type of feedback ('rating', 'thumbs', 'detailed')
            feedback_value (Any): The feedback value
            session_id (str): Session identifier
        ...
        try:
            if self.feedback_manager:
                self.feedback_manager.add_message_feedback(
                    message_id=message_id,
                    session_id=session_id,
                    feedback_type=feedback_type,
                    feedback_value=feedback_value
                )
                logger.info("Feedback recorded: {feedback_type} for message {message_id}")
                return True
            return False
        except Exception as error:
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        """
        Get learning insights and analytics from the feedback system.
        ...
        try:
            ...
            return {'error': 'Feedback manager not available'}
        except Exception as error:
            logger.error("Failed to get learning insights: {e}")
            return {'error': str(e)}

    def _get_model_recommendations(self) -> Dict[str, Any]:
        """Get AI model recommendations based on performance data."""
        try:
            ...
            for style in ConversationStyle:
                best_model = self.learning_service.get_optimal_model(
                    conversation_style=style.value,
                    available_models=available_model_names
                )
                if best_model:
                    recommendations[style.value] = best_model

            return recommendations
        except Exception as error:
            logger.error("Failed to get model recommendations: {e}")
            return {}
