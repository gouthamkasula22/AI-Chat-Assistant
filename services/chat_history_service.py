...
class ChatHistoryService:
    ...
    def start_or_resume_conversation(self, session_id: str, 
        ...
        try:
            ...
            if conversation:
                logger.info(f"Resuming conversation {conversation['id']} for session {session_id}")
                return conversation
            ...
            conversation = self.db_manager.get_conversation_by_session(session_id)
            logger.info(f"Started new conversation {conversation_id} for session {session_id}")
            
            return conversation
            
        except Exception as e:
            logger.error(f"Failed to start/resume conversation: {e}")
            raise
        ...
        try:
            for file_path in glob.glob(session_files_pattern):
                try:
                    ...
                    migrated_count += 1
                    logger.info(f"Migrated session file: {file_path}")
